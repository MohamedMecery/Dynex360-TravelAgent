-- TravelOS: Geography Schema (Countries, Cities, Destinations)
-- Version: 2.0 Expanded MVP
-- Note: countries and cities are GLOBAL reference data (no tenant_id, no soft delete).
--       destinations are TENANT-scoped, soft-deletable, audited.
...
-- Destinations (tenant-scoped, curated)
...
CREATE UNIQUE INDEX uq_destinations_tenant_slug ON destinations(tenant_id, slug) WHERE deleted_at IS NULL;
