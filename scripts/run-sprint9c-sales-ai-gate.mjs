/**
 * Sprint 9C — AI Sales Assistant gate.
 *
 * Usage: node scripts/run-sprint9c-sales-ai-gate.mjs
 */
...
async function main() {
  console.log("Sprint 9C AI Sales Assistant gate\n");
  const admin = adminClient();

  const tables = [
    "ai_sales_snapshots",
    "ai_sales_score_history",
    "ai_sales_recommendations",
    "ai_sales_recommendation_feedback",
    "ai_sales_insight_cache",
  ];
  ...
  const { error: rpcError } = await admin.rpc("get_sales_insights", {
    p_from: new Date(Date.now() - 30 * 86400000).toISOString(),
    p_to: new Date().toISOString(),
  });
  record(
    "RPC",
    "get_sales_insights callable",
  ...
  const { data: perm } = await admin
    ...
    .select("id")
    .eq("module", "ai")
    .eq("action", "sales.read")
    .maybeSingle();
  record("Permissions", "ai.sales.read", perm ? "PASS" : "FAIL");
  ...
  record(
    "AI Platform",
    "sales agent_key seeded",
    agentRow?.length ? "PASS" : "SKIP",
    agentRow?.length ? undefined : "apply migration 059"
  );

  const outDir = resolve(root, "scripts");
  mkdirSync(outDir, { recursive: true });
  writeFileSync(
    resolve(outDir, "sprint9c-sales-ai-gate-results.json"),
