...
import { fetchOperationsInsights } from "@/lib/operations-ai/recommendation-service";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (!hasAiPermission(gate.access.role, "ai.operations.insights.read")) {
    return NextResponse.json(
      { error: { code: "FORBIDDEN", message: "Missing ai.operations.insights.read permission" } },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const data = await fetchOperationsInsights(gate.supabase, from, to);
