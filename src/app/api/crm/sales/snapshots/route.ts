...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const entityType = request.nextUrl.searchParams.get("entity_type") as SalesEntityType | null;
  ...
  if (!entityType || !entityId) {
    ...
    if (entityType && entityIds?.length) {
      const { data, error } = await gate.supabase
        ...
        .select(
          "entity_id, entity_type, health_score, priority_score, win_probability, priority_tier, confidence, computed_at"
        )
